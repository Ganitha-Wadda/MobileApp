...
import Svg, {
  Ellipse, Circle, Polygon, Polyline, Rect, G,
...
function CoinIcon() {
  ...
  useEffect(() => {
    Animated.loop(
      Animated.sequence([
        Animated.timing(bounce, { toValue: -4, duration: 700, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
  ...
  return (
    <Animated.View style={{ transform: [{ translateY: bounce }] }}>
      <Svg width={38} height={32} viewBox="0 0 54 46">
        ...
        <Circle cx="27" cy="20" r="14" fill="none" stroke="#F5B820" strokeWidth="1.5" />
        <Ellipse cx="21" cy="15" rx="3.5" ry="2" fill="rgba(255,255,255,0.5)" />
      </Svg>
      <Animated.View style={[styles.starOverlay, { transform: [{ rotate }] }]}>
        <Svg width={14} height={14} viewBox="0 0 20 20">
          <Polygon points="10,1 12.2,7.5 19,7.5 13.5,11.5 15.8,18 10,14 4.2,18 6.5,11.5 1,7.5 7.8,7.5" fill="#F5A800" opacity="0.95" />
        </Svg>
      </Animated.View>
    </Animated.View>
  );
...
function MedalIcon() {
  ...
  const rotate = swing.interpolate({ inputRange: [-1, 1], outputRange: ["-7deg", "7deg"] });

  return (
    <Animated.View style={{ transform: [{ rotate }] }}>
      <Svg width={32} height={38} viewBox="0 0 46 54">
        ...
        <Polygon points="23,28 25.2,34.5 32,34.5 26.5,38.5 28.8,45 23,41 17.2,45 19.5,38.5 14,34.5 20.8,34.5" fill="#F59E0B" stroke="#D97706" strokeWidth="0.5" />
...
function PapersIcon() {
  ...
  useEffect(() => {
    Animated.loop(
      Animated.sequence([
        Animated.timing(bob, { toValue: -4, duration: 800, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
        Animated.timing(bob, { toValue: 0,  duration: 800, easing: Easing.inOut(Easing.sin), useNativeDriver: true }),
      ])
    ).start();
    Animated.timing(check, { toValue: 1, duration: 800, delay: 600, easing: Easing.out(Easing.quad), useNativeDriver: false }).start();
  }, []);

  const strokeDash = check.interpolate({ inputRange: [0, 1], outputRange: [20, 0] });
  ...
  return (
    <Animated.View style={{ transform: [{ translateY: bob }] }}>
      <Svg width={36} height={36} viewBox="0 0 50 50">
        ...
        <Rect x="8"  y="10" width="28" height="34" rx="4" fill="white" stroke="#BFDBFE" strokeWidth="1" />
        ...
        <Polyline points="26,36 29,39.5 35,30.5" stroke="white" strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round" fill="none" strokeDasharray="20" strokeDashoffset={strokeDash} />
...
function Sparkles({ color }) {
  const dots = [
    ...
    { top: "12%", right: "8%", size: 5, delay: 500  },
    { top: "55%", left: "7%",  size: 3, delay: 1100 },
    { top: "60%", right: "6%", size: 4, delay: 800  },
    { top: "28%", left: "46%", size: 3, delay: 1600 },
  ];
  return (
    <>
      {dots.map((d, i) => <SparkDot key={i} {...d} color={color} index={i} />)}
    </>
  );
}
...
function SparkDot({ top, left, right, size, delay, color, index }) {
  ...
  const scale      = anim.interpolate({ inputRange: [0, 0.5, 1], outputRange: [0, 1.3, 0.2] });
...
function StatCard({ label, value, icon, sparkleColor, delay = 0 }) {
  ...
  useEffect(() => {
    Animated.spring(pop, { toValue: 1, delay, friction: 6, tension: 120, useNativeDriver: true }).start();
  }, []);

  const scale = pop.interpolate({ inputRange: [0, 1], outputRange: [0.6, 1] });

  return (
    <Animated.View style={[styles.card, { transform: [{ scale }], opacity: pop }]}>
...
export default function Hero() {
  return (
    <View style={styles.container}>
      <StatCard label="Coins"            value={20} icon={<CoinIcon />}   sparkleColor="#FFD700" delay={60}  />
      <StatCard label="Rank"             value={1}  icon={<MedalIcon />}  sparkleColor="#FFB020" delay={180} />
      <StatCard label="Completed Papers" value={1}  icon={<PapersIcon />} sparkleColor="#60AEFF" delay={300} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  card: {
    flex: 1,
    backgroundColor: "#5047eb",
    borderRadius: 14,            // ← slightly tighter
    borderWidth: 1.5,
